...
Im Folgenden wird erläutert, wie eine \acs{aas} von Grund auf modelliert, schrittweise mit Inhalten gefüllt und abschließend in Form einer AASX-Datei gespeichert bzw. exportiert werden kann.  

\medskip
\noindent
\textbf{Erstellung einer neuen AAS}\\
...
Abbildung~\ref{fig:NeuesAASPaket} zeigt den initialen Aufbau, der im weiteren Verlauf sukzessive um Submodelle und deren Inhalte erweitert wird.
Dateien, wie ein Titelbild für die robocell, sind dabei ein wichtiger Bestandteil und können direkt in die \acs{aas}-Umgebung eingebettet werden.
...
Anschließend müssen die Metadaten für die \acs{aas} sowie das zugehörige Asset eingetragen werden.
...
\noindent
\textbf{Submodelle}\\
Im nächsten Schritt der Modellierung müssen die benötigten Submodelle zur \acs{aas} hinzugefügt werden. 
Auch diese lassen sich entweder als Instanz oder Typ anlegen.
Während Instanzen konkrete Ausprägungen eines Submodells darstellen und mit spezifischen Werten befüllt werden, dienen Typen als wiederverwendbare Vorlagen für gleichartige Strukturen. 
Der Package Explorer unterstützt beide Varianten, was eine flexible Modellierung je nach Anwendungsfall erlaubt.
...
Alternativ können standardisierte Submodel Templates verwendet werden, wie sie beispielsweise über das Repository der \acs{idta} \cite{idtaTemplates} als AASX-Dateien bereitgestellt werden.
Diese lassen sich über ein sogenanntes Auxiliary AAS in die Modellierungsumgebung importieren und dort weiterverwenden.
Diese Templates beeinhalten bereits eine vordefinierte Struktur mit typischen Submodellelementen, wodurch der Modellierungsaufwand erheblich reduziert wird.

Nach dem Hinzufügen der gewünschten Submodellen können die enthaltenen Submodellelemente mit konkreten Inhalten wie Werten, Dateien, Referenzen oder Beziehungen befüllt werden.
Besonders bei der Verwendung von Templates sollte darauf geachtet werden, dass die Struktur dieser nicht verändert wird, da dies sonst zu Abweichungen von der Submodellspezifikation führen kann.
...
\begin{figure}[htbp]
    \centering
    \includegraphics{Bilder/ModellierungAAS/V1/ConceptDescription.png}
    \caption{Package Explorer: Data Specification Template}
