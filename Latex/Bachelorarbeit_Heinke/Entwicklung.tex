...
Zur exemplarischen Abbildung und dynamischen Berechnung des \acs{pcf} der Gesamtmaschine wird das von der \acs{idta} spezifizierte \acs{smt} Carbon Footprint \cite{SpezifikaitonPCF} eingesetzt.  
...
Die Komponenten wurden als Typ-1-\acs{aas} zur Verfügung gestellt, wobei jede \acs{aas} ein eigenes Submodell gemäß des \acs{smt} Carbon Footprint enthält.

Um diese in die \acs{pcf}-Berechnung einzubeziehen, können sie direkt im Carbon Footprint-Submodell der robocell referenziert werden. 
...
Alternativ ist auch eine Referenzierung über das \acs{bom}-Submodell möglich, allerdings erschwert dies die spätere Aggregation über die AAS Web UI.
