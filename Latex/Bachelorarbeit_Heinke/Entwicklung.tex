...
Wie in Kapitel~\ref{sec: Sicherheit} beschrieben, sieht die Security-Spezifikation der \acs{idta} die Anwendung eines attributbasierten Zugriffskontrollmodells (\acs{abac}) vor. 
Dieses ermöglicht eine feingranulare Steuerung des Zugriffs auf Submodelle und deren Elemente anhand von differenzierten Nutzerattributen.

Da die \acs{abac}-Funktionalität in aktuellen Umsetzungen wie dem AASX Server Blazor oder Eclipse BaSyx noch nicht vollständig unterstützt wird, erfolgt die Realisierung im Rahmen dieses Anwendungsfalls exemplarisch auf Basis eines rollenbasierten Ansatzes (\acs{rbac}), wie er von der Eclipse BaSyx-Plattform unterstützt wird. 

Zur Umsetzung der Authentifizierung und Autorisierung wird Keycloak eingesetzt \cite{Keycloak}. 
Der Identity Provider ermöglicht die zentrale Verwaltung von Nutzern, Rollen und Clients und stellt \acsp{jwt} für die Zugriffskontrolle aus. 
Keycloak steht dabei wie alle BaSyx-Komponenten auch als Docker-Container zur Verfügung, wodurch eine nahtlose Einbindung in das bestehende System möglich ist.

Im ersten Schritt muss ein dediziertes Realm eingerichtet werden, das als isolierte Umgebung zur Verwaltung sämtlicher projektbezogener Identitäten dient. 
Dies kann direkt über die Keycloak-Benutzeroberfläche erfolgen.
In dem Realm lassen sich unter anderem Benutzerkonten sowie Rollen definieren.
Die Zuweisung dieser zueinander erfolgt über sogenannte Role Mappings innerhalb des Realms.
Ein Nutzer kann dabei mehreren Rollen gleichzeitig zugeordnet sein. 
Jedes Benutzerkonto erhält zudem ein individuelles Passwort zur Authentifizierung.

In diesem Anwendungsfall werden beispielhaft folgende Benutzer mit den zugehörigen Rollen eingerichtet:
...
Neben menschlichen Benutzern können mit Keycloak auch technische Clients, die z.\,B. technische Dienste oder Anwendungen repräsentieren, verwaltet . 
Diese Clients verfügen über eigene Service Accounts, denen ebenfalls Rollen zugewiesen werden können. 
So kann beispielsweise ein Client mit der Rolle Kunde ausgestattet werden und somit im Namen dieser Rolle auf geschützte Ressourcen zugreifen.
...
In diesem Anwendungsfall erhält die Rolle Groninger-Mitarbeiter beispielsweise vollen Zugriff auf sämtliche Submodelle des digitalen Produktpasses, während die Rolle Kunde lediglich Leserechte auf ausgewählte Submodelle wie Typenschild oder Technische Daten hat.
...
Eine exemplarische \acs{rbac}-Konfiguration für die Rolle Groninger-Mitarbeiter in der AAS Environment ist in Listing~\ref{lst:rbac-config} dargestellt.

!!! Bilder

Wie kann man sich als Nutzer anmelden über AAS Web UI \\
Mit API \\
