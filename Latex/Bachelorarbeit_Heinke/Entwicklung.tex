...
Mit dem Befehl 
\tcbox[inlinebox]{\textbf{aas\_test\_engine check\_files}} 
...
\begin{figure}[htbp]
    ...
    \caption{Aufbau des SMT Time Series Data}
...
Die kontrollierte Bereitstellung der im \acs{dpp} enthaltenen Informationen ist essenziell, insbesondere im Hinblick auf Datenschutz, regulatorische Anforderungen sowie den Schutz geistigen Eigentums. 
Wie in Kapitel~\ref{sec: Sicherheit} beschrieben, sieht die Security-Spezifikation der \acs{idta} die Nutzung eines attributbasierten Zugriffskontrollmodells (\acs{abac}) vor. 

Da die \acs{abac}-Funktionalität in aktuellen Umsetzungen wie dem AASX Server Blazor oder Eclipse BaSyx jedoch noch nicht vollständig unterstützt wird, erfolgt die Realisierung in diesem Anwendungsfall exemplarisch auf Basis eines rollenbasierten Ansatzes (\acs{rbac}), wie er von der Eclipse BaSyx-Plattform unterstützt wird. 
Die Zugriffskontrolle erfolgt dabei durch die Zuweisung von Rechten an vordefinierte Rollen, denen wiederum Benutzer oder technische Clients zugeordnet werden können.

Zur Umsetzung der Authentifizierung und Autorisierung wird der Open-Source Identity Provider Keycloak eingesetzt \cite{Keycloak}.
Dieser ermöglicht die zentrale Verwaltung von Nutzern, Rollen und Clients und unterstützt eine tokenbasierte Zugriffskontrolle mithilfe von \acsp{jwt}.
Keycloak steht als Docker-Container zur Verfügung und lässt sich nahtlos in die bestehende BaSyx-Systemarchitektur integrieren.

Im ersten Schritt muss ein dediziertes Realm eingerichtet werden, das als gekapselte Umgebung sämtlicher projektbezogener Identitäten dient. 
Innerhalb des Realms lassen sich Benutzer als auch Rollen definieren.
Die Zuweisung erfolgt über sogenannte Role Mappings.
Ein Nutzer kann entweder einer einzelnen oder mehreren Rollen gleichzeitig zugeordnet sein.

Jedes Benutzerkonto erhält dabei einen eindeutigen Benutzernamen sowie ein individuelles Passwort zur Authentifizierung. 
Für erhöhte Sicherheit kann das initial vergebene Passwort als temporär markiert werden, sodass der Benutzer beim ersten Login zu dessen Änderung gezwungen wird. 
Darüber hinaus bietet Keycloak ebenfalls die Möglichkeit, sogenannte Required Actions zu definieren.
Diese verpflichten einen Nutzer beim nächsten Login zur Durchführung bestimmter Aktionen, etwa zur Verifizierung der E-Mail-Adresse oder zur Aktualisierung des Passworts.
Auf diese Weise lässt sich der Authentifizierungsprozess flexibel an projektspezifische und sicherheitsrelevante Vorgaben anpassen.

In diesem Anwendungsfall werden beispielhaft folgende Benutzer eingerichtet:
...
Neben der Verwaltung menschlicher Benutzern erlaubt Keycloak auch die Administration technischer Clients, die beispielsweise maschinelle Anwendungen oder externe Systeme repräsentieren. 
Die Identifikation der Clients erfolgt über eine eindeutige Client-ID sowie die Nutzung des OpenID Connect-Protokolls.
Die Authentifizierung wird in der Regel durch eine Kombination aus Client-ID und einem geheimen Schlüssel (Client-Secret) sichergestellt.
Für jeden Client kann ein sogenannter Service Account aktiviert werden, der als technisches Benutzerkonto agiert und, analog zu menschlichen Nutzern, mit spezifischen Rollen ausgestattet werden kann. 
Dies ermöglicht eine kontrollierte und differenzierte Zugriffsteuerung auch für automatisierte Systemzugriffe.

Die Rechtevergabe innerhalb von Eclipse BaSyx erfolgt rollenbasiert über externe komponentenspezifische Konfigurationsdateien, typischerweise im JSON-Format. 
Diese definieren, welchen Rollen welche Rechte auf bestimmte Elemente innerhalb der BaSyx-Umgebung erhalten, darunter AAS, Submodelle und Concept Descriptions. 
Die möglichen Aktionen umfassen z.\,B. READ, WRITE oder DELETE. 
Für generische Regeln kann das Platzhalterzeichen \texttt{*} verwendet werden.  
So lässt sich etwa definieren, dass eine Rolle Zugriff auf alle AAS oder alle Submodelle innerhalb einer BaSyx-Komponente erhält.
Für die AAS Web UI ist keine seperate \acs{rbac}-Datei nötig, sie orientiert sich an den Konfigurationen der zugrundeliegenden Services.
...
Der Rolle Service-Techniker hingegen werden Lese- und Schreibrechte auf wartungsrelevante Submodelle (z.,B. Wartung oder Kontrollkomponente) zugewiesen, jedoch kein Zugriff auf das Submodell \acs{pcf}, welches für diese Rolle nicht relevant ist.
Eine exemplarische \acs{rbac}-Konfiguration für die Rolle Groninger-Mitarbeiter in der AAS Environment ist in Listing~\ref{lst:groningerMitarbeiterRBAC} dargestellt.
...
Ist \acs{rbac} aktiviert, wird der Benutzer beim Aufruf der Weboberfläche automatisch zur Keycloak-Anmeldeseite weitergeleitet. 
Dort erfolgt die Authentifizierung durch Eingabe von Benutzername und Passwort. 
Die AAS Web UI ist in Keycloak als eigener Client registriert und erhält nach erfolgreicher Anmeldung ein Zugriffstoken, das die Rolleninformationen des Nutzers enthält. 
Dieses Token wird im Hintergrund verwendet, um auf die zugrundeliegenden BaSyx-Komponenten wie die AAS Environment oder den Discovery Service zuzugreifen. 
Die Entscheidung über die Zugriffsberechtigung trifft dabei nicht die AAS Web UI selbst, sondern die angesprochenen Dienste, die das übermittelte Token auslesen und die darin enthaltenen Rollen mit den in den jeweiligen \acs{rbac}-Konfigurationsdateien definierten Berechtigungen abgleichen.

Alternativ kann auch direkt über die \acs{api} auf einzelne BaSyx-Komponenten zugegriffen werden, beispielsweise durch technische Clients oder externe Anwendungen. 
Im Unterschied zur Nutzung über die AAS Web UI muss das Zugriffstoken in diesem Fall aktiv vom jeweiligen Client bei Keycloak angefordert werden, etwa über den Token-Endpunkt des entsprechenden Realms. 
Um sich gegenüber den Komponenten zu authentifizieren, kann das erhaltene Token anschließend im Authorization-Header der HTTP-Anfrage mitgesendet werden. 
Die Entscheidung über die Zugriffsberechtigung erfolgt dabei analog zur AAS Web UI durch die jeweiligen Dienste auf Basis der im Token enthaltenen Rollen und der hinterlegten \acs{rbac}-Regeln.
