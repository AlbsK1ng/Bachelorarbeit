...
Als Identity Provider kommt eine Anwendung namens Keycloak zum Einsatz. 
...
Keycloak als Identity Provider (JWT-Token) -> neues Realm groninger-aas
...
Dieses ermöglicht eine feingranulare Regelung des Zugriffs auf Submodelle und deren Elemente anhand von differenzierten Nutzerattributen.
...
Die Anwendung steht dabei als Docker-Container zur Verfügung, wodurch eine nahtlose Einbindung in das bestehende BaSyx-System möglich ist.

Im ersten Schritt muss ein dediziertes Realm erstellt werden, in dem sämtliche für das Projekt erforderlichen Identitäten verwaltet werden. 
Exemplarisch werden drei Benutzer mit jeweils unterschiedlichen Rollen konfiguriert.
Jeder Nutzer erhält dabei ein individuelles Passwort, mit dem er sich später authentifizieren kann.

\begin{itemize}[noitemsep, leftmargin=*]
    \item \textbf{groninger.meyer} (Rolle: \textit{Groninger-Mitarbeiter}) 
    \item \textbf{customer.doe} (Rolle: \textit{Kunde})
    \item \textbf{technician.john} (Rolle: \textit{Service-Techniker})
\end{itemize}

Neben Benutzern, die reale Personen repräsentieren, existieren in Keycloak auch Clients, die technische Anwendungen oder Dienste repräsentieren.
Diese Clients nutzen sogenannte Service Accounts, denen gezielt Rollen zugewiesen werden können.
Auf diese Weise lassen sich auch automatisierte Systemzugriffe kontrollieren und absichern.
Beispielsweise wird dem Client \texttt{client-customer} die Rolle \textit{Kunde} zugewiesen, wodurch dieser im späteren Zugriffskontext dieselben Berechtigungen wie ein (menschlicher) Nutzer mit dieser Rolle erhält.

Die eigentlichen Zugriffsrechte auf die Inhalte einer \acs{aas} werden in separaten JSON-Dateien definiert.
Diese regeln den Zugriff auf die AAS Environment, Registries und den Discovery Service.
Dabei wird jeder Rolle ein Satz von Berechtigungen wie Lesen, Schreiben oder Aktualisieren zugewiesen.
Zusätzlich können bestimmte AAS, Submodelle oder Concept Descriptions spezifiziert werden, auf die diese Regeln angewendet werden sollen.
Sollen die Regeln nicht nur für einzelne, sondern für alle Elemente eines bestimmten Typs gelten, kann dafür das Platzhalterzeichen '*' verwendet werden.

Außerdem können bestimmte AAS bzw. Submodell spezigiziert werden auf die diese Reglen angenwadt werden sollen.
'*' ist eine Wildcard und zählt für alle. 


Für AAS Environment, Registries und Discovery Serivce.
Dabei können den verschiedenen Rollen unterschiedliche Rechte zugeweisen werden.
Der Mitarbeiter hält zum Beispiel vollen Zugriff auf alle Submodelle des DPP bzw. der AAS, während der Kunde nur bestimmmte Submodelle lesen darf.
Beispiel links und rechts nebeneinander als Code Aussshnit vlt?
Außerdem angeben für welche AAS, Submodelle oder Concept DescriptionsVlt hier ein JSON-Beispiel?
