...
Im ersten Schritt wird gezeigt, wie ein unternehmensspezifisches Submodel Template erstellt und anschließend in ein Typ-Submodell überführt werden kann.
Dieses bildet die Grundlage für die automatisierte Erzeugung eines Instanz-Submodells, das mit konkreten Daten befüllt, in eine \acs{aas}-Instanz eingebettet und schließlich in das BaSyx-System integriert wird.

\subsubsection{Arbeiten mit Submodel Templates}
\label{chap:ErstellenvonSubmodelTemplates}
Die Verwendung von Submodel Templates spielt eine zentrale Rolle für den effizienten Einsatz digitaler Zwillinge in Industrie-4.0-Anwendungen. 
Ein einmal definiertes Template kann zur Erstellung eines Typ-Submodells genutzt werden, das wiederum als Vorlage für beliebig viele \acs{aas}-Instanzen dient. 
Auf diese Weise wird nicht nur die Konsistenz der Datenstruktur sichergestellt, sondern auch der Aufwand für die Modellierung signifikant reduziert.

Ausgangspunkt für die Erstellung eines Typ-Submodells ist in der Regel ein generisches Submodel Template. 
Im folgenden Beispiel wird das Template Technische Daten \cite{SpezifikaitonTechnischeDaten} verwendet, das eine übergeordnete Struktur zur Beschreibung technischer Merkmale eines Produkts bereitstellt. 
Es enthält grundlegende Kategorien wie Generelle Informationen, Technische Informationen oder Produktklassifikation. 
Diese dienen als Rahmen, sind jedoch noch nicht mit konkreten inhaltlichen Ausprägungen versehen.

Auf Unternehmensebene kann dieses Template produktspezifisch angepasst werden, beispielsweise für einen bestimmten Maschinentyp wie die robocell. 
In der Praxis wäre auch eine branchenspezifische Ableitung denkbar, diese wird hier jedoch nicht weiter betrachtet. 
Für die technische Umsetzung kann der Package Explorer genutzt werden, über den das Template importiert und erweitert werden kann. 
So lassen sich beispielsweise produktspezifische Anforderungen wie Umgebungsbedingungen oder elektrische Daten ergänzen.
Das resultierende unternehmensspezifische Template erhält eine eigene semanticId, bleibt jedoch zusätzlich mit der ursprünglichen semanticId des generischen Templates verknüpft, um die semantische Rückverfolgbarkeit zur standardisierten Struktur sicherzustellen.
...
Auch hier sollte die Verbindung zur ursprünglichen bzw. unternehmensspezifischen Template-Struktur über entsprechende semanticId-Verknüpfungen beibehalten werden.
