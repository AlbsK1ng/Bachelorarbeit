\newpage
\section{Entwicklung}
In diesem Kapitel wird die schrittweise Entwicklung des digitalen Zwillings beschrieben.
Das zugrundeliegende Asset ist das Abfüll- und Verschließmodul der robocell-Linie.
Dabei wird zunächst eine konzeptionelle Basis erarbeitet, die als Ausgangspunkt für die anschließende Implementierung des digitalen Zwilings dient.
Darauf aufbauend folgt die statische Modellierung mithilfe der \acs{aas} und des Package Explorers sowie die Validierung mit einer Test Engine.
Außerdem wird gezeigt, wie die \acs{aas} mit der Eclipse BaSyx-Plattform bereitgestelllt und um dynamische Informationen ergänzt werden kann.
Dabei werden sowohl Echtzeidaten über \acs{opcua} als auch Zeitreihendaten über eine InfluxDB eingebunden.

Abschließend wird auf die Entwicklung von drei exemplarischen Anwendungsfällen eingegangen.
...
Hierzu wird zunächst im Package Explorer ein Submodel Template erstellt, das anschließend in ein Typ-Submodell überführt wird. 
Dieses dient als Grundlage für die automatisierte Erzeugung eines Instanz-Submodells, das mit konkreten Daten befüllt, in eine \acs{aas}-Instanz eingebettet und schließlich in ein Industrie-4.0-System integriert wird.

\subsubsection{Arbeiten mit Submodel Templates}
\label{chap:ErstellenvonSubmodelTemplates}
Das Erstellen von Submodel Templates spielt eine zentrale Rolle für den effizienten Einsatz digitaler Zwillinge in Industrie 4.0-Anwendungen. 
Ein einmal definiertes Template kann für beliebig viele \acs{aas}-Instanzen wiederverwendet werden. 
...
Ein solches Template kann entweder vollständig neu modelliert oder von bestehenden Submodel Templates abgeleitet werden, wie es im Folgenden exemplarisch anhand des Submodells Technische Daten \cite{SpezifikaitonTechnischeDaten} gezeigt wird.
Dieses stellt eine standardisierte Struktur zur Beschreibung technischer Merkmale eines Produkts bereit.
Dabei handelt es sich um ein generisches Template, das lediglich grundlegende Strukturkategorien vorgibt.
Es enthält beispielsweise \acsp{smc} wie Generelle Informationen, Technische Informationen oder Produktklassifikation, die allerdings noch nicht mit konkreten Eigenschaften gefüllt sind.

Auf Unternehmensebene kann dieses Template für eine bestimmte Produktgruppe durch zusätzliche Merkmale individualisiert werden. 
Für die praktische Umsetzung kann der Package Explorer verwendet werden, über den das Template importiert und angepasst werden kann. 
So lassen sich beispielsweise produktspezifische Anforderungen wie Umgebungsbedingungen oder elektrische Daten ergänzen. 
Das resultierende unternehmensspezifische Template erhält eine eigene semanticId, bleibt jedoch zusätzlich mit der ursprünglichen semanticId des Templaets verknüpft, um die Rückverfolgbarkeit zu diesem sicherzustellen.

Aus dem angepassten Template kann anschließend ein Typ-Submodell erstellt werden, das bereits allgemeingültige Merkmale einer Produktgruppe enthält. 
Dieses dient als Vorlage für die spätere Erstellung konkreter AAS-Instanzen, die mit produktspezifischen Werten vervollständigt werden. 
Auch hier wird die Verbindung zur ursprünglichen bzw. unternehmensspezifschen Tmeplate-Struktur über die entsprechenden semtaicId-Verknüpfungen beibehalten.


%
Im Rahmen der Entwicklung digitaler Zwillinge spielt die strukturierte Ableitung von Typ-Submodellen eine zentrale Rolle. Ziel ist es, aus einem generischen Submodel Template schrittweise eine unternehmensspezifische, wiederverwendbare Struktur zu entwickeln, die als Grundlage für die automatisierte Erstellung konkreter \acs{aas}-Instanzen dient.

Der Prozess beginnt mit einem generischen Template, das in der Regel durch ein Standardisierungsgremium oder eine übergeordnete Spezifikation, wie etwa die Verwaltungsschalen-Spezifikation \cite{SpezifikaitonTechnischeDaten}, bereitgestellt wird.
Dieses Template definiert die grundlegende Struktur eines Submodells, etwa durch standardisierte Submodel Element Collections (SMCs) wie „Generelle Informationen“, „Technische Merkmale“ oder „Produktklassifikation“. 
Zu diesem Zeitpunkt handelt es sich um eine generische Vorlage ohne branchenspezifische Inhalte.

In einem zweiten Schritt erfolgt die branchenspezifische Verfeinerung dieses Templates, typischerweise durch einen Arbeitskreis, Industrieverband oder ein Konsortium. 
Ziel dieser Ebene ist es, die generische Struktur um fachspezifische Inhalte zu erweitern – etwa besondere Sicherheitsanforderungen oder Schnittstellenspezifikationen für bestimmte Maschinenarten wie Werkzeugmaschinen, Verpackungsanlagen oder Antriebssysteme. Das so entstehende branchenspezifische Template bildet eine verbindliche Struktur für eine bestimmte Domäne und ist weiterhin über seine \texttt{semanticId} mit dem ursprünglichen generischen Template verknüpft.

Darauf aufbauend kann ein Unternehmen dieses branchenspezifische Template übernehmen und weiter individualisieren. Dabei werden z.\,B. interne Anforderungen ergänzt, produkt- oder standortspezifische Eigenschaften definiert oder eigene Klassifikationssysteme integriert. Dieses unternehmensspezifische Template erhält ebenfalls eine eigene \texttt{semanticId}, bleibt jedoch über die semantische Referenzierung mit den vorhergehenden Template-Stufen verbunden. Dadurch bleibt die Rückverfolgbarkeit zur branchenspezifischen und generischen Vorlage gewahrt, während gleichzeitig eine hohe Flexibilität für unternehmensinterne Anforderungen besteht.

Aus dem angepassten Template wird im letzten Schritt ein Typ-Submodell abgeleitet. Dieses enthält alle für eine bestimmte Produktgruppe relevanten Strukturinformationen, jedoch noch keine konkreten Werte. Es stellt eine instanziierbare Vorlage dar, die bei der Erzeugung konkreter AAS-Instanzen verwendet wird. Die eigentliche Instanziierung erfolgt durch eine automatisierte Befüllung mit produktspezifischen Daten, wie sie im nachfolgenden Kapitel beschrieben wird.
%
...
\subsubsection{Konzept eines KI-Modells zur Anomalieerkennung}
Datenquelle
...
\subsubsection{Verwaltung der KI Metadaten in der \acs{aas}}
