...
Im Mittelpunkt steht der \acs{aas}-Demonstrator, der die prototypische Umsetzung des digitalen Zwillings des Abfüll- und Verschließmoduls der robocell-Linie zeigt.
...
Neben der Haupt-\acs{aas} des Abfüll- und Verschließmoduls sind auch exemplarische \acs{aas} untergeordneter Komponenten enthalten, wodurch eine modulare und hierarchisch aufgebaute digitale Repräsentation entsteht.
Als Laufzeitumgebung kommt Eclipse BaSyx zum Einsatz, während für die Datenanbindung standardisierte Protokolle wie \acs{opcua} sowie \acs{http}/\acs{rest} verwendet werden.
...
Die Architektur besteht aus mehreren Komponenten, die einen aktiven \mbox{digitalen Zwilling} im Sinne einer Typ-2-\acs{aas} ermöglichen.
...
Der modulare Aufbau erleichtert Erweiterungen und Anpassungen und schafft damit eine skalierbare Infrastruktur für unterschiedliche Anforderungen.

Die Bereitstellung der \acs{aas} erfolgt in der Eclipse BaSyx-Plattform.
Der ursprünglich vorgesehene AASX Server Blazor wurde zugunsten der BaSyx-Plattform ersetzt, da diese mehr Flexibilität und zusätzliche Funktionen für die Umsetzung digitaler Zwillinge bietet.
Abbildung~\ref{fig:BaSyxArchitektur} zeigt den Aufbau des Systems, welches als zentrales BaSyx-Backend die Verwaltung sämtlicher \acs{aas}-Bestandteile übernimmt.
...
Der Discovery Service ist für die Auflösung einer Asset-\acs{id} (globalAssetId) in die zugehörige \acs{aas}-\acs{id} zuständig und ermöglicht so das gezielte Auffinden einer \acs{aas} im Gesamtsystem.
Die Registries dienen als Verzeichnisse aller registrierten \acs{aas} und \mbox{Submodelle} mit ihren jeweiligen Endpunkten innerhalb der AAS Environment.
Die \acs{aas} Registry kann optional auch Referenzen auf die Submodel Registry (SM Registry) enthalten.

Die \acs{aas} Environment beherbergt die eigentlichen Inhalte und ist in drei \mbox{Repositories} gegliedert. 
Im AAS Repository werden die \acs{aas} einschließlich Asset-Metadaten und Referenzen auf Submodelle verwaltet. 
Das Submodel Repository (SM Repository) speichert die Submodelle samt Inhalten und Referenzen auf die zugehörigen \mbox{Concept Descriptions}. 
Im \acs{cd} Repository sind die Concept Descriptions abgelegt, die die semantischen Beschreibungen bereitstellen.
...
Zudem sind sie an eine gemeinsame MongoDB-Instanz angebunden, in der sämtliche Daten persistent gespeichert werden.
...
\linebreak
Datengenerator simulierten Werte mithilfe von Telegraf fortlaufend in eine InfluxDB geschrieben, wodurch eine historische Datenbasis aufgebaut wird.
...
% \vspace{-0.5em}
Zur Weiterverarbeitung und Visualisierung der statischen Inhalte der \acs{aas} sowie der durch die dynamische Erweiterung ergänzten Echtzeit- und Zeitreihendaten kommt die AAS Web UI zum Einsatz.
...
Bei Zeitreihendaten ist nicht der eigentliche Dateninhalt in der \acs{aas} gespeichert, sondern lediglich ein Verweis auf den Endpunkt der InfluxDB.
Die AAS Web UI kann diesen nutzen, um die historischen Daten aus der Datenbank abzurufen und in der Web-Oberfläche zu visualisieren.

\subsubsection{Statische Submodelle}
Die statische Modellierung des Demonstrators erfolgte ausschließlich im \mbox{Package~Explorer}.
Dazu wurden alle relevanten \acs{aas} sowie die zugehörigen Submodelle angelegt, mit Inhalten befüllt, semantisch angereichert und mit eindeutigen Identifikatoren versehen.
...
Statische \mbox{Submodelle} sind darin mit einer Nummer gekennzeichnet, die der im Folgenden verwendeten Gliederung entspricht. 
...
Die Beziehungen zwischen den Komponenten werden über RelationshipElements mit einer eindeutigen semanticId gemäß dem \acs{smt} Hierarchical Structures Enabling Bills of Material \cite{SpezifikationHierachischeStrukturen} hergestellt.
...
\pagebreak
~die Betriebsanleitung oder die Netzwerkübersicht, während das Submodell 3D-Modelle \acs{cad}-Daten zur Geometrie beinhaltet. 
Alle Dateien sind als File-Elemente direkt in die Struktur der \acs{aas} eingebettet und jeweils in eigenen \acsp{smc} organisiert, die zusätzlich Metainformationen zu Identifikation, Klassifikation und Versionierung enthalten. 
...
Ergänzend dazu stellt die Nutzung von \mbox{Concept Descriptions} in Kombination mit standardisierten \acsp{smt} sicher, dass die Inhalte von unterschiedlichen Systemen oder Personen gleichermaßen interpretierbar und nutzbar sind.
Auf diese Weise wird nicht nur die Interoperabilität gefördert, sondern auch die Rückverfolgbarkeit von Maschinendaten über den gesamten Lebenszyklus gestärkt.
...
Die Zustände orientieren sich am \acs{packml}-Standard, einem von der Organization for Machine Automation and Control entwickelten Modell zur einheitlichen Beschreibung von Maschinenzuständen und zulässigen Zustandsübergängen, insbesondere für Verpackungsmaschinen \cite{OMAC}. 
...
Mithilfe des \mbox{Discovery Service} werden deren \acs{aas} identifiziert und vorhandene \acs{cf}-Submodelle ausgelesen. 
Die enthaltenen \acs{pcf}-Werte werden anschließend zu Gesamtwerten für die einzelnen Lebenszyklusphasen aggregiert und in das \acs{cf}-Submodell der Haupt-\acs{aas} zurückgeschrieben.
...
Bei Verfügbarkeit weiterer Komponenten-\acs{aas} lässt sich die Liste unkompliziert ergänzen, sodass sukzessive die gesamte Maschine berücksichtigt werden kann. 
Perspektivisch ist auch eine Erweiterung auf weitere Lebenszyklusphasen, wie Nutzung oder Entsorgung, sowie die Einbeziehung des \acs{tcf} denkbar, um eine ganzheitliche ökologische Bilanzierung zu ermöglichen.

Auf diese Weise wird nicht nur die Transparenz gestärkt, sondern auch die Rückverfolgbarkeit, da die einzelnen Beiträge durch die Einbindung separater Komponenten-\acs{aas} eindeutig zuordenbar sind. 
...
Dadurch verringerte sich der ansonsten sehr aufwendige manuelle Aufbau eigener \mbox{Concept Descriptions} deutlich, was den Modellierungsprozess spürbar beschleunigte.
