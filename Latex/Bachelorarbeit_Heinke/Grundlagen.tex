...
Gerade wenn Informationen aus der \acs{aas} über die Grenzen des eigenen Unternehmens hinweg bereitgestellt werden, ist es essenziell, die Sicherheit der enthaltenen Daten zu gewährleisten. 
Die neueste Spezifikation Part 4: Security \cite{SpezifikationPart4} der \acs{idta} bildet hierfür das technische und konzeptionelle Fundament.
Sehr neu - neue Ansätze und Services
Identity provider - policy storage etc. 
Darin wird ein umfassendes Access Control Model beschrieben, das auf dem Prinzip der attributbasierten Zugriffskontrolle (Attribute Based Access Control (ABAC)) basiert.
ABAC ermöglicht dabei eine feingranulare Steuerung von Zugriffsrechten für bestimmte Operationen (z.B. Lesen oder Schreiben), die an einem Objekt ausgeführt werden sollen.
Im Kontext der \acs{aas} können diese Objekte beispielsweise ein Submodell oder ein bestimmtes Submodellelemt wie ein Merkmal (Property) sein.
Die Zugriffsentscheidung erfolgt dabei über Attribute. 
Diese umfassen unteranderem Subjektattribute (wer stellt die Anfrage?), Objektattribute (auf welche Informationen soll zugegriffen werden?), die beabsichtigte Aktion (z. B. Lesen oder Schreiben) sowie kontextbezogene Merkmale wie der aktuelle Zeitpunkt oder der Zustand des Systems.
Zusätzlich können auch Rollen als Attribute definiert werden, wodurch sich der ABAC Ansatz auch flexibel mit rollenbasierter Zugriffskontrolle (Role Based Access Controll (RBAC)) kombinieren lässt.
