...
% Grundsätzlich wird dabei zwischen zwei unterschiedlichen Lernmethoden unterschieden.
% Beim überwachten Lernen wird einem Algorithmus ein Datensatz zur Verfügung gestellt, der sowohl Eingabewerte als auch die dazugehörigen Ziel- bzw. Ausgabewerte enthält.
% Das Modell kann während des Trainings Zusammenhänge und Muster zwischen diesen Daten erkennen und daraus eine Funktion ableiten, die neue Eingaben korrekt zuordnet.
% Typische Anwendungsbereiche sind die Klassifikation (z. B. die Einteilung von Bildern in Kategorien) und die Regression (z. B. die Vorhersage numerischer Werte wie Temperatur oder Energieverbrauch).
% Demgegenüber steht das unüberwachte Lernen, bei dem das Modell mit ungelabelten Daten trainiert wird. 
% Es liegen also keine Zielwerte vor, und der Algorithmus muss eigenständig Muster oder Strukturen in den Eingabedaten identifizieren. 
% Häufig kommt dabei die Clusteranalyse zum Einsatz, bei der ähnliche Datenpunkte zu Gruppen (Clustern) zusammengefasst werden. 
% Eine typische Anwendung ist die Anomalieerkennung, bei der Abweichungen von einem erwarteten Normalverhalten erkannt werden können.
% \cite{DLDefinition}

% Ein vielversprechender Ansatz innerhalb des maschinellen Lernens ist das sogenannte Deep Learning, dass sowohl im überwachten als auch unüberwachten Kontext zum Einsatz kommt.
% Es basiert auf künstlichen neuronalen Netzen, die in ihrer Struktur an das Verhalten von Neuronen im menschlichen Gehirn angelehnt sind.
% Während klassische maschinelle Lernmodelle meist nur aus einer oder wenigen Schichten bestehen, verwendet Deep Learning sogenannte tiefe neuronale Netze, die aus einer Vielzahl aufeinanderfolgender Schichten bestehen.
...
% Eine besondere Form des Deep Learnings sind generative Modelle. 
% Sie werden mit großen Datenmengen trainiert, erkennen darin komplexe Muster und sind in der Lage, daraus neue, realistische Daten zu erzeugen [Quelle Springer].
% Neben der Generierung können sie aber auch zur Analyse und Bewertung von Daten eingesetzt werden.
% Dies ist insbesondere bei industriellen Szenarien von Bedeutung, beispielsweise bei der Erkennung von Anomalien in Zeitreihendaten.
% Häufig kommt dabei ein Autoencoder zum Einsatz. 
...
% durch mehrfaches Wiederholen ein spezifisches Problem lösen können.
% Im Gegensatz zu herkömmlichen Computerprogrammen wird hierbei kein Lösungsweg vorgegeben. 
% Das Modell ist vielmehr selbsständig in der Lage Muster in den vorgegebenen Daten zu erkennen und anhand dieser eine Lösung für ein spezifisches Problem ableiten. für ein  basierend darauf Vorhersagen zu tätigen [Quelle Website].
% Aufgrund seiner zentralen Bedeutung für zahlreiche Anwendungen gilt maschinelles Lernen häufig auch als Schlüsseltechnologie der \acs{ki} [Quelle Paper]. 
...
% Maschinelles Lernen -> Neuronale Netzte / Deep Learning -> Spezialform des Deep Learning ist generative KI -> generative KI wird genutz in Anwendung Anomaliererkennung z.B. Autoencoder / die Frage ist ob auch in Predictive Maintenance


% Insbesondere in Forschung und Wirtschaft gewinnt \acs{ki} zunehmend an Bedeutung. 
% Unternehmen setzen vermehrt KI-basierte Anwendungen ein, um komplexe Aufgaben wie die vorausschauende Wartung (Predictive Maintenance), die Erkennung von Anomalien oder die Automatisierung anspruchsvoller Arbeitsabläufe zu meistern.
% https://digital-zentral.de/die-rolle-von-kuenstlicher-intelligenz-in-der-industrie-4-0/
...
\begin{figure}[htbp]
    ...
    \caption{Klassifizierung digitaler Zwilling}
    \label{fig:klassifizierungDT}
\end{figure}
...
Durch die Kombination eines Assets mit seiner \acs{aas} entsteht eine sogenannte Industrie-4.0-Komponente.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\textwidth]{Bilder/aas_blau.pdf}
...
Die einfachste Möglichkeit besteht im Dateiaustausch. Hierfür wurden speziell für die \acs{aas} sogenannte AASX-Dateien \cite{SpezifikationPart5} entwickelt, die den einfachen Austausch statischer \acs{aas} (Typ 1) ermöglichen.
...
Eine dezentrale Lösung zur Umsetzung bildet der von der ZVEI vorgestellte \ac{dpp40} \cite{DPP40}.
Der \acs{dpp40} basiert dabei auf zwei etablierten Standards. Zum Einen das digitale Typenschild, und zum Anderen die AAS (siehe auch Kapitel~\nameref{chap:AAS}).
...
Organisiert werden die Daten im \acs{dpp40} in verschiedenen Submodellen der AAS. 
...
Während beispielsweise das digitale Typenschild (DN) oder der \acs{pcf} öffentlich zugänglich sind, werden sensible Informationen wie technische Dokumentationen oder sicherheitsrelevante Details nur bestimmten autorisierten Gruppen zugänglich gemacht.
Eine schematische Darstellung des \acs{dpp40} in Abbildung \ref{fig:dpp40} zu sehen.

% \vspace{0em}
% \begin{figure}[htbp]
%     \centering
%     \includegraphics[width=0.75\textwidth]{Bilder/DPP/DppKonzeptNeu.pdf}
%     \caption{Konzept des \acs{dpp40} (eigene Darstellung in Anlehnung an \cite{DPP40Bild})}
%     \label{fig:dpp40}
% \end{figure}
...
\begin{figure}[htbp]
    ...
    \caption{robocell Abfüll- und Verschließmodul !!! Quelle Betriebsanleitung fehlt}
    \label{fig:robocell}
\end{figure}
...
Neben der lokalen Modellierung erlaubt das Tool ebenfalls die Verbindung zu einem AAS-Server über standardisierte Schnittstellen (z.B. \acs{opcua} oder HTTP/REST).
